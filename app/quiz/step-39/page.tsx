...
function Step39Content() {
  ...
  useEffect(() => {
    const handleMouseLeave = (e: MouseEvent) => {
      if (e.clientY <= 0 && !hasRedirected.current) {
        ...
        router.push(`/quiz/step-40?${params.toString()}`)
      }
    }
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      if (
        ...
        ((e.ctrlKey && (e.key === "w" || e.key === "t")) || (e.altKey && e.key === "F4") || e.key === "F5")
      ) {
        hasRedirected.current = true
        const params = new URLSearchParams(searchParams)
        router.push(`/quiz/step-40?${params.toString()}`)
  ...
  const handleGetMyPlan = () => {
    hasRedirected.current = true // Disable exit intent when user clicks checkout
    const url = checkoutUrls[selectedPlan as keyof typeof checkoutUrls]
    window.open(url, "_blank")
  }

  // Get images based on gender and age
  ...
  return (
    <div className="min-h-screen w-full bg-white flex flex-col">
      ...
      <header className="w-full px-4 sm:px-6 py-4 flex justify-between items-center bg-white border-b border-gray-100 flex-shrink-0">
        <Link href={`/quiz/step-38?${searchParams.toString()}`} className="p-2">
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 py-4 sm:py-8">
          ...
          <div className="text-center mb-8 sm:mb-12">
            ...
            <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-bold text-gray-900 mb-4 sm:mb-6 px-2">
              Get your personalized plan and start feeling better today
            </h1>
            <p className="text-base sm:text-lg md:text-xl text-gray-600 max-w-3xl mx-auto px-2">
              Join over 1,000,000 people who have transformed their well-being with our scientifically-backed approach
            </p>
          </div>
